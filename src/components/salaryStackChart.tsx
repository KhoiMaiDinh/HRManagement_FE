...
function SalaryStackChart() {
    ...
    type TIndexAxis = "x" | "y" | undefined;
    type TAlign = "center" | "start" | "end" | undefined;
    const align: TAlign = "end";
    const options = {
        plugins: {
            title: {
                display: false,
        ...
        indexAxis: "x" as TIndexAxis, // Add this line to switch to horizontal bar chart
            ...
            y: {
                ...
                ticks: {
                    callback: (
                        ...
                        if (typeof tickValue === "number" && tickValue <= 100)
                            return `${tickValue}%`;
                    },
                },
                grace: 0,
        ...
        const getData = async () => {
            try {
                ...
                res.data.map((month) => {
                    ...
                    tempOffice.push(
                        (month.officeSalaries / month.firmSalaries) * 100
                    );
                    tempSales.push(
                        (month.salesSalaries / month.firmSalaries) * 100
                    );
                    tempProduction.push(
                        (month.productionSalaries / month.firmSalaries) * 100
                    );
                });
